...
🚧 **Early / under construction, but three real modules now, plus the shared reference material they all lean on.** Tech stack and deploy pipeline are decided (Phase 1); DHCP (Phase 2), DNS, and Email Authentication (both Phase 4) are each a full Lab + Case + Classification Drill, browsable at `/dhcp/`, `/dns/`, and `/email-auth/`; `/concepts/` holds the cross-cutting patterns (Signal vs. Silence, Eventual Consistency, Fail-Open vs. Fail-Closed, Trust-Boundary Flattening) and named diagnostic methodology that every module links into instead of re-explaining. Five more domains from the concept doc are still unbuilt. See [ROADMAP.md](ROADMAP.md) for the working plan and [it-troubleshooting-platform-concept.md](it-troubleshooting-platform-concept.md) for the full design concept this is built from.
...
- `dhcp/`, `dns/`, `email-auth/` — domain modules: `index.html` (module landing) plus one page per content item (Lab, Case, Drill). The pattern the rest of Phase 4's domains will follow.
