...
- `src/islands/case-dns-split-horizon/` and `src/islands/drill-dns-nxdomain/` — the DNS module's Case and Drill (Phase 4), both linking back to `/concepts/cross-cutting-concepts.html` rather than re-explaining Signal vs. Silence

All four domain-module islands are structurally identical to the two Phase 1 proof islands — different content, same shape — confirming the islands pattern generalizes rather than being a one-off.
...
**Content pages live in a per-domain folder at the repo root** — `dhcp/` and `dns/` are the pattern for the rest (`ad-gpo/`, ... as Phase 4 continues): a module `index.html` linking to one page per content item, Labs as plain static HTML/CSS, Cases and Drills as a thin static page mounting the matching island. **`concepts/`** holds the cross-cutting reference material (Signal vs. Silence, Eventual Consistency, Fail-Open vs. Fail-Closed, Trust-Boundary Flattening, and the named diagnostic methodology) — every domain module should link into it rather than re-explaining a pattern from scratch; retrofit older modules when a new one's content overlaps with a concept they already used implicitly. `it-troubleshooting-platform-concept.md` is the source for all of this — write new domain content from there, not from scratch.
